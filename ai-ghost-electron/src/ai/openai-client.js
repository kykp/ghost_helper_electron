...
ФОРМАТ ОТВЕТА — верни СТРОГО валидный JSON-объект (без markdown-обёртки вокруг него). ПОРЯДОК ПОЛЕЙ ОБЯЗАТЕЛЕН: сначала "isAddon", потом "short", потом "detailed":
{
  "isAddon": false,
...
Поле "isAddon" — флаг для UI; стоит ПЕРВЫМ, чтобы UI знал стиль блока до того, как придёт текст. Оставляй false для новых вопросов; ставь true только в режиме «продолжается та же тема» (см. ниже).
...
ЕСЛИ ПРОДОЛЖАЕТСЯ ТА ЖЕ ТЕМА:
В истории сообщений перед твоим запросом могут быть твои предыдущие ответы (assistant) в виде JSON {"short":"...","detailed":"..."} — это подсказки, которые ты уже дал по этому же диалогу. Если новый вопрос или реплика — это тот же вопрос, его повтор, переформулировка или продолжение темы, и прошлая подсказка её уже раскрывает:
- НЕ повторяй то, что уже сказал в "short"/"detailed" прошлого ответа — даже другими словами
- Верни ТОЛЬКО дельту: упущенный нюанс, новый угол, важная оговорка, исправление, более точный пример кода, ответ на уточняющий подвопрос
- "short" в этом режиме оформляй как продолжение: «Ещё важно:…», «Уточнение:…», «Альтернатива:…», «Стоит добавить:…», «На самом деле:…»
- В JSON-ответе ОБЯЗАТЕЛЬНО поставь "isAddon": true — этим UI отметит подсказку как продолжение прошлой темы (другой цвет, отступ слева)
- Если добавить нечего — верни {"short":"—","detailed":""} и подсказка не покажется
- Если интервьюер задал НОВЫЙ вопрос (другая тема) — отвечай по обычным правилам, "isAddon": false

ПРИМЕР режима дополнения (та же тема):
Прошлый ответ (в истории): {"isAddon":false,"short":"undefined — переменная объявлена, но не присвоена; null — явно заданное «нет значения».","detailed":"..."}
Новый вопрос (Собеседник): "А typeof что вернёт для обоих?"
Правильный ответ:
{"isAddon": true, "short": "Уточнение: typeof undefined → 'undefined', а typeof null → 'object' — известная историческая странность языка.", "detailed": "..."}

ПРИМЕР нового вопроса (НЕ дополнение):
Прошлый ответ (в истории): {"isAddon":false,"short":"undefined — переменная объявлена, но не присвоена; null — явно заданное «нет значения».","detailed":"..."}
Новый вопрос (Собеседник): "Что такое замыкание?"
Правильный ответ:
{"isAddon": false, "short": "Замыкание — функция, которая запомнила переменные из области, где её создали.", "detailed": "..."}

Ещё раз: если тема последнего вопроса УЖЕ была раскрыта в одном из твоих прошлых ответов в истории — это режим дополнения, "isAddon": true ОБЯЗАТЕЛЬНО.
...
- Повторять собственные прошлые ответы из истории — см. раздел «Если продолжается та же тема»
...
- Выдумывать проблему по коду, если код на скриншоте не виден или не читается.
  ...
  {"short":"—","detailed":""} и ничего не придумывай.`;
...
- Если на скриншотах нет задачи по коду — верни {"short":"На скриншотах не видно задачи по коду","detailed":""}.
...
class OpenAIClient {
  ...
  async getHint({ dialog, reason }, onProgress) {
    ...
    if (hint && (hint.short || hint.detailed)) {
      ...
      // дельту, а не пересказывать всё заново. Detailed подрезаем по
      // длине — иначе при большом коде контекст разбухает.
      const detailedCap = 1200;
